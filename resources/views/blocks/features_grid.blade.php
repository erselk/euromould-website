    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="text-center max-w-3xl mx-auto mb-16">
            @if(isset($data['title']) && $data['title'])
                <h2 class="text-3xl md:text-4xl font-black text-slate-900 mb-6 tracking-tight">{{ $data['title'] }}</h2>
            @endif
        ...
        @if(isset($data['features']))
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                @foreach($data['features'] as $feature)
                    <div class="bg-white p-10 rounded-xl shadow-sm border border-slate-100 hover:shadow-lg hover:border-slate-200 transition-all duration-300">
