            ...
            @foreach($services as $service)
                <a href="{{ localized_url('/' . $service->slug) }}" class="group flex flex-col h-full bg-slate-50 border border-slate-100 hover:border-slate-300 transition-all duration-300">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow">
                        <h3 class="text-2xl font-bold text-slate-900 mb-4 group-hover:text-primary transition-colors">{{ __($service->title) }}</h3>
                        <p class="text-slate-600 leading-relaxed text-sm mb-6 line-clamp-4">{{ __($service->description) }}</p>
