...
<section class="relative bg-slate-900 text-white py-20 overflow-hidden">
    ...
    @if(isset($data['bg_image']))
        <div class="absolute inset-0 z-0">
            <img src="{{ Storage::url($data['bg_image']) }}" alt="{{ $data['title'] ?? '' }}" class="w-full h-full object-cover opacity-30">
        </div>
    @endif
    <!-- Gradient & Overlay -->
    ...
    <div class="relative z-10 container mx-auto px-6 max-w-7xl">
        <div class="text-center max-w-3xl mx-auto">
            @if(isset($data['subtitle']) && $data['subtitle'])
                <p class="text-primary font-bold tracking-[0.2em] uppercase text-sm mb-4 animate-fade-in-up">{{ $data['subtitle'] }}</p>
            @endif
